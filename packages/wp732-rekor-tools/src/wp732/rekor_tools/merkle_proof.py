...
class Hasher:
    ...
    def hash_children(self, left_node, right_node):
        h = self.new()
        b = bytes([RFC6962_NODE_HASH_PREFIX]) + left_node + right_node
        h.update(b)
        return h.digest()
...
def verify_consistency(hasher, size1, size2, proof, root1, root2):
    ...
    if size1 == 0:
        if bytearray_proof:
            raise ValueError(
                f"expected empty bytearray_proof,"
...
def root_from_inclusion_proof(hasher, index, size, leaf_hash, proof):
    ...
    if len(leaf_hash) != hasher.size():
        raise ValueError(
            f"leaf_hash has unexpected size {len(leaf_hash)},"
            f" want {hasher.size()}"
        )
